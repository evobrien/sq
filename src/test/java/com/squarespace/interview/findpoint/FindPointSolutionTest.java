...
public class FindPointSolutionTest {
  ...
  /**
   * Runs the current implementation the FindPointSolution problem with the
   * inputs of the given input data and copares them to the outputs of the
   * given expected data.
   */
  ...
  /**
   * Given a file name on the classpath of expected results parse it into a
   * data structure to use for validating results. The file contains a given
   * (x,y) point and the path to that point in a view hierarchy.
   *
  ...
  /**
   * Given a {@link InputStream} providing a view hierarchy definition in JSON
   * and a set of points, provide the path through the hierarchy for each given
   * point.
   *
   * @param hierarchy JSON file defining a nested view hierarchy.
   * @param pointsToFind A unique collection of points to find within the view
   *      hierarchy.
   * @return An ordered list of String node IDs that define the path to the
   *      view containing each given point.
   */
  private static Map<Point, List<String>> processInput(InputStream hierarchy, Set<Point> pointsToFind) {
    Node rootNode;
    try (JsonReader reader = Json.createReader(hierarchy)) {
    ...
    for (Point toFind : pointsToFind) {
      List<String> result = runActualImplementation(rootNode, toFind);
  ...
  /**
   * Initializes a {@link Node} instance from the given {@link JsonObject}.
   *
   * @param jsonObject A reference to the JSON blob containing node data.
   ...
   */
  private static List<String> runActualImplementation(Node rootNode, Point toFind) {
    try {
      return FindPointSolutionKt.findPathToNode(rootNode, toFind);
    } catch (NotImplementedError e) {
      return FindPointSolutionJava.findPathToNode(rootNode, toFind);
    }
  }
}
